...
public class HybridSort<T> implements Sort<T> {
     ...
     */
    public void quickSort(SortList<T> sortList, int left, int right) { // 6/6 points (only gets all points if quick sort and partition are implemented correctly)
        if(left < right) {
            if((right - left) < k-1) {
                insertionSort(sortList, left, right);
            } else {
                int x = partition(sortList, left, right);
                quickSort(sortList, left, x);
                quickSort(sortList, x + 1, right);
            }
        }
    }
    ...
    public int partition(SortList<T> sortList, int left, int right) { // 6/6 points
        ...
        while(p < p2) {
            do {
                p++;
            } while (p <= right && comparator.compare(sortList.get(p), pivot) < 0);
            do {
                p2--;
            } while (p2 >= left && comparator.compare(sortList.get(p2), pivot) > 0);

            if (p < p2) {
                T temp = sortList.get(p);
                sortList.set(p, sortList.get(p2));
                sortList.set(p2, temp);
            }
        }
        return p2;
    }
     ...
     * @param right The rightmost index of the list to be sorted.
     */
    public void insertionSort(SortList<T> sortList, int left, int right) { // 6/6 points
